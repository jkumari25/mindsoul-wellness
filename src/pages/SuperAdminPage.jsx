...
const pill = (ok) => (
  <span
    className={`px-2 py-0.5 rounded-full text-xs font-semibold ${ok ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"}`}
  >
    {ok ? "Yes" : "No"}
  </span>
);
...
function StatCard({ label, value, icon, bg }) {
  return (
    <div className="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm border border-gray-100">
      <div
        className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl flex-shrink-0 ${bg}`}
      >
        {icon}
      </div>
...
function Avatar({
  ...
  return src ? (
    ...
    <div
      className={`${sz} ${bg} rounded-full flex items-center justify-center font-bold flex-shrink-0`}
    >
      {letter}
    </div>
  );
...
function SectionLabel({ children }) {
  return (
    <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-3 mt-6 first:mt-0">
      {children}
    </p>
  );
}
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-1">
      {items.map((t, i) => (
        <span
          key={i}
          className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${c}`}
        >
          {t}
        </span>
      ))}
...
function CounsellorList({ onSelect }) {
  ...
  const filtered = list.filter((c) => {
    ...
    return [c.email, c.firstName, c.lastName].some((s) =>
        ...
        <div className="space-y-2">
          {filtered.map((c) => (
            ...
            >
              ...
              <span className="text-gray-300 group-hover:text-indigo-400 transition flex-shrink-0">
                ›
              </span>
            </button>
          ))}
...
function TagInput({ label, items, onChange, color = "indigo" }) {
  ...
  return (
    <div>
      ...
      <div
        className="flex flex-wrap gap-1.5 min-h-[42px] border border-gray-200 rounded-xl px-2.5 py-2 bg-gray-50 focus-within:ring-2 focus-within:ring-indigo-400 focus-within:border-transparent cursor-text"
        onClick={(e) => e.currentTarget.querySelector("input")?.focus()}
      >
        {items.map((t, i) => (
          <span
            key={i}
            className={`flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium border ${colorMap[color]}`}
          >
            {t}
            <button
              type="button"
...
function CounsellorProfile({ counsellor: initial, onBack, onDeleted }) {
  ...
  const fileInputRef = useCallback((el) => {
    if (el) el.value = "";
  }, []);
  ...
  return (
    <div className="space-y-0">
        ...
        <div className="bg-red-50 border border-red-200 rounded-2xl p-5 mb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2 flex-shrink-0">
            <button
              ...
              className="px-4 py-2 border border-red-200 text-red-600 rounded-xl text-sm font-medium hover:bg-white transition"
            >
              Cancel
            </button>
            <button
              onClick={doDelete}
              disabled={deleting}
              className="px-4 py-2 bg-red-600 text-white rounded-xl text-sm font-medium hover:bg-red-700 transition disabled:opacity-60"
            >
      ...
      {success && (
        <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-sm rounded-2xl px-4 py-3 mb-4">
          {success}
        </div>
      )}
      {error && (
          ...
          <div className="-mt-10 sm:-mt-14 mb-4 flex items-end justify-between">
            {editing ? (
              <label className="cursor-pointer group relative flex-shrink-0">
                ...
                {imageFile && (
                  <p className="text-xs text-indigo-600 mt-1 text-center font-medium">
                    {imageFile.name}
                  </p>
                )}
              </label>
              ...
              <Avatar
                ...
                size="xl"
                bg="bg-white text-indigo-600 border-4 border-white shadow-lg"
              />
            )}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
                ...
                <InfoRow label="Experience">
                  {data.experience ? `${data.experience} years` : null}
                </InfoRow>
                <InfoRow label="Base Price">₹{data.basePrice || 0}</InfoRow>
                <InfoRow label="Session Price (with fee)">
                  ₹{data.sessionPrice || 0}
                </InfoRow>
                <InfoRow label="Platform Fee">₹{data.platformFee || 0}</InfoRow>
                <InfoRow label="Slot Duration">
                  {data.slotDuration ? `${data.slotDuration} min` : null}
                </InfoRow>
                <InfoRow label="Joined">{fmtDate(data.createdAt)}</InfoRow>
                <InfoRow label="Last Updated">
                  {fmtDate(data.updatedAt)}
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Tags items={data.expertise} color="indigo" />
                </div>
                <div>
                  <p className="text-xs text-gray-400 mb-1">Focus Areas</p>
                  <Tags items={data.focusAreas} color="green" />
                </div>
                <div>
                  <p className="text-xs text-gray-400 mb-1">Languages</p>
                  <Tags items={data.languages} color="yellow" />
                </div>
              </div>
...
function UserProfile({ user, onBack }) {
  return (
    <div className="space-y-0">
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-gray-500 hover:text-gray-800 font-medium text-sm transition mb-6"
      >
        ...
        <div className="px-6 pb-6">
          <div className="-mt-10 sm:-mt-14 mb-4">
            <Avatar
              ...
              size="xl"
              bg="bg-white text-purple-600 border-4 border-white shadow-lg"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
            ...
            <InfoRow label="Profile Completed">
              {pill(user.profileCompleted)}
            </InfoRow>
...
function Appointments() {
  ...
  const filtered = list.filter((a) => {
    const q = search.toLowerCase();
    return [a.counsellorName, a.studentName, a.studentEmail, a.status].some(
              ...
              <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${statusColor(a.status)}`}
                    >
                      {a.status || "unknown"}
                    </span>
                    {a.amount > 0 && (
                  ...
                  <p className="font-semibold text-gray-900 truncate">
                    {a.counsellorName || a.counsellorEmail} →{" "}
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col lg:flex-row">
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-24 lg:pb-0">
        <main className="flex-1 px-4 lg:px-8 py-6 lg:py-8 max-w-5xl w-full mx-auto">
          ...
          {subview?.type === "user" && (
            <UserProfile user={subview.data} onBack={() => setSubview(null)} />
          )}
          {!subview && tab === "overview" && <Overview />}
          {!subview && tab === "counsellors" && (
            <CounsellorList
              onSelect={(data) => setSubview({ type: "counsellor", data })}
            />
          )}
          {!subview && tab === "users" && (
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 bg-white border-t border-gray-100 flex z-20 safe-area-pb">
        {TABS.map((t) => (
          ...
          >
            <span
              className={`${tab === t.id ? "scale-110" : ""} transition-transform`}
            >
              {t.icon}
            </span>
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="inline-flex w-16 h-16 bg-indigo-600 rounded-2xl items-center justify-center text-white text-2xl font-bold mb-5 shadow-lg shadow-indigo-200">
            M
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Super Admin</h1>
          <p className="text-gray-400 text-sm mt-1">
            Mindsoul Wellness Platform
          </p>
        </div>
