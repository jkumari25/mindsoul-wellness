...
export default function Navbar() {
  const [isOpen, setIsOpen] = useState(false);
  const [loginOpen, setLoginOpen] = useState(false); // Counselor Login
  const [userLoginOpen, setUserLoginOpen] = useState(false); // User Login
  const [otpOpen, setOtpOpen] = useState(false); // OTP MODAL
  ...
  return (
    <>
      ...
      <CounselorLogin
        isOpen={loginOpen}
        onClose={() => setLoginOpen(false)}
        openOtpModal={() => setOtpOpen(true)} // 👈 IMPORTANT
      />
