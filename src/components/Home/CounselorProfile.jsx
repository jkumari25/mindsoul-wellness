...
export default function CounselorProfile() {
  ...
  // 🔵 SAFE WORKING HOURS
  const workingHours = counsellor.workingHours || {};
      ...
      <div className="flex-1 space-y-8">
        ...
        <div className="bg-white shadow-md rounded-2xl p-6 border border-gray-200">
                ...
                {counsellor.experience && (
                  <div className="flex items-center gap-1">
                    ...
                    {(() => {
                      ...
                      return /year/i.test(exp) ? exp : `${exp}+ Years`;
                    })()}
                  </div>
                )}
          ...
          <div className="mt-8">
            ...
            <div className="p-4 border-2 border-primary rounded-xl text-left lg:text-center bg-purple-50 font-body text-lg w-auto lg:w-[20%]">
          ...
          {Object.keys(workingHours).length > 0 && (
            <div className="mt-6">
              ...
              {workingHours.morning && (
                <p className="text-gray-700">
                  Morning: {workingHours.morning?.start || "N/A"} -{" "}
                  {workingHours.morning?.end || "N/A"}
                </p>
              )}

              {workingHours.afternoon && (
                <p className="text-gray-700 mt-1">
                  Afternoon: {workingHours.afternoon?.start || "N/A"} -{" "}
                  {workingHours.afternoon?.end || "N/A"}
                </p>
              )}

              {workingHours.evening && (
                <p className="text-gray-700 mt-1">
                  Evening: {workingHours.evening?.start || "N/A"} -{" "}
                  {workingHours.evening?.end || "N/A"}
                </p>
              )}
            </div>
          )}
