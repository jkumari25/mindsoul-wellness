...
import React, { useState } from "react";
import { FaHeart, FaPlay, FaGoogle } from "react-icons/fa";
import LoginPage from "./LoginPage"; // ✅ added
...
export default function Registration({
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // ✅ switch to login instead of closing
      setShowLogin(true);
    } catch (err) {
      console.error(err);
      setError("Something went wrong. Try again.");
      ...
      {isOpen && !showLogin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-3">
          <div className="relative bg-transparent shadow-2xl w-full max-w-[850px] rounded-2xl overflow-hidden flex flex-col md:flex-row text-white animate-fadeIn">
            ...
            {/* Right Section */}
            <div className="flex flex-col items-center justify-center bg-[#16181C] w-full md:w-[45%] p-8   space-y-5 relative">
              <button
                onClick={onClose}
                className="absolute top-3 right-4 text-gray-400 hover:text-white text-2xl"
              >
              ...
              <div className="text-left space-y-2 w-[285px]">
                ...
                <p className="text-[14px] text-gray-400 leading-relaxed">
                  Join MindSoul Wellness to explore emotional healing and
                  growth.
                </p>
              </div>
              ...
              <div className="flex flex-col items-center gap-3">
                <p
                  onClick={() => setShowLogin(true)} // ✅ FIXED
