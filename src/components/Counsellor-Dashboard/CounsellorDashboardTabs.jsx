  ...
  return (
    <div className="max-w-7xl mx-auto mt-6 md:mt-10 px-4">
      ...
      <div className="overflow-x-auto">
        <div className="flex gap-2 bg-gray-100 p-1 rounded-xl w-max min-w-full md:w-fit">
          {[
            { key: TABS.INFO, label: "My Info" },
            { key: TABS.APPOINTMENTS, label: "Appointments" },
            { key: TABS.WEEKLY_SCHEDULE, label: "Weekly Schedule" },
            ...
            <button
              ...
              className={`px-4 md:px-5 py-2 text-sm md:text-md whitespace-nowrap rounded-lg font-medium transition
              ${
                activeTab === tab.key
                  ? "bg-white text-textDark shadow"
                  : "text-gray-500 hover:text-indigo-600"
      ...
      <div className="mt-6 md:mt-8">
        {/* INFO TAB */}
        {activeTab === TABS.INFO && counsellorData && (
          <div className="bg-white rounded-2xl shadow-md p-4 md:p-6 grid grid-cols-1 md:grid-cols-[280px_1fr] gap-6">
            {/* LEFT COLUMN */}
            <div className="flex flex-col items-center md:items-start text-center md:text-left">
              <img
                src={counsellorData.imageUrl}
              ...
              <p className="text-gray-500 text-sm md:text-md break-all">
            ...
            <div className="space-y-5 text-sm md:text-base">
              ...
              <Section title="Working Days & Hours">
                <ul className="space-y-1 text-sm md:text-md text-gray-600">
                  {counsellorData.workingDays.map((day, idx) => {
                    const h = counsellorData.workingHours;
                    return (
                      <li key={idx}>
        ...
        {/* APPOINTMENTS */}
        {activeTab === TABS.APPOINTMENTS && (
          <div className="bg-white rounded-xl shadow p-4 md:p-10 text-center text-gray-500">
            <CounsellorAppointments />
          </div>
        )}
        ...
        {/* WEEKLY SCHEDULE */}
        {activeTab === TABS.WEEKLY_SCHEDULE && (
          <div className="bg-white rounded-xl shadow p-4 md:p-10">
            <CounsellorWeeklySchedule />
          </div>
        )}
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((item, idx) => (
          <span
            key={idx}
            className={`${colorClasses[color]} px-3 py-1 rounded-full text-xs md:text-sm`}
          >
            {item}
          </span>
        ))}
